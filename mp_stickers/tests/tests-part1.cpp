...
TEST_CASE("Image scale(500, 200) scales pixel data in a reasonable way", "[weight=1][part=1]") {
  Image img = createRainbowImage();
  
  Image result = createRainbowImage();
  result.scale(500, 200);

  REQUIRE( result.getPixel(499, 137).h == img.getPixel(359, 99).h );
  REQUIRE( result.getPixel(0, 0).h == img.getPixel(0, 0).h );
  REQUIRE( result.getPixel(0, 137).h == img.getPixel(0, 99).h );
  REQUIRE( result.getPixel(499, 0).h == img.getPixel(359, 0).h );
}
