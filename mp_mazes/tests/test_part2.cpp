...
// TEST_CASE("testMazeConstructorBasic") {
// 	SquareMaze maze;
// 	maze.makeMaze(2, 2);
// 	PNG* output = maze.drawMaze();
// 	output->writeToFile("testMazeConstructorBasic" + string(".png"));
// }

TEST_CASE("testCanTravelBasic") {
	PNG solnImage = READ_UNSOLVED_PNG("testDrawMazeSmall", 2, 2);
	MazeReader soln(solnImage);
	SquareMaze maze;
	copyMaze(soln, &maze);

	REQUIRE(maze.canTravel(0, 0, 0) == true);
	REQUIRE(maze.canTravel(0, 0, 1) == false);
	REQUIRE(maze.canTravel(0, 0, 2) == false);
	REQUIRE(maze.canTravel(0, 0, 3) == false);

	REQUIRE(maze.canTravel(1, 0, 0) == false);
	REQUIRE(maze.canTravel(1, 0, 1) == true);
	REQUIRE(maze.canTravel(1, 0, 2) == true);
	REQUIRE(maze.canTravel(1, 0, 3) == false);

	REQUIRE(maze.canTravel(0, 1, 0) == true);
	REQUIRE(maze.canTravel(0, 1, 1) == false);
	REQUIRE(maze.canTravel(0, 1, 2) == false);
	REQUIRE(maze.canTravel(0, 1, 3) == false);

	REQUIRE(maze.canTravel(1, 1, 0) == false);
	REQUIRE(maze.canTravel(1, 1, 1) == false);
	REQUIRE(maze.canTravel(1, 1, 2) == true);
	REQUIRE(maze.canTravel(1, 1, 3) == true);
}
...
TEST_CASE("testMakeMazeConnectedBasic", "[weight=10][part2]") {
	SquareMaze maze;
	maze.makeMaze(2, 2);
	// PNG* output = maze.drawMaze();
	// output->writeToFile("testMakeMazeConnectedBasic" + string(".png"));
	assert_maze_connected(maze, 2, 2);
}
...
TEST_CASE("testMakeMazeConnected", "[weight=10][part2]") {
	...
	maze.makeMaze(15, 15);
	// PNG* output = maze.drawMaze();
	// output->writeToFile("testMakeMazeConnected" + string(".png"));
