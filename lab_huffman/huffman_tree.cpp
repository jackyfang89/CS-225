...
HuffmanTree::TreeNode* HuffmanTree::removeSmallest(queue<TreeNode*>& singleQueue, queue<TreeNode*>& mergeQueue)
{
    TreeNode* smallest;
    ...
    smallest = (singleQueue.front()->freq.getFrequency() <= mergeQueue.front()->freq.getFrequency()) ? singleQueue.front() : mergeQueue.front();

    smallest == singleQueue.front() ? singleQueue.pop() : mergeQueue.pop();
...
{
    if (frequencies.size() == 0) return; //do nothing
    if (frequencies.size() == 1) {
        root_ = new TreeNode(frequencies.at(0));
        return;
    }
    ...
    size_t total = singleQueue.size() + mergeQueue.size();

    while (total > 1) {
        TreeNode* a = removeSmallest(singleQueue, mergeQueue);
        TreeNode* b = removeSmallest(singleQueue, mergeQueue);
        
        TreeNode* internal = new TreeNode(a->freq.getFrequency() + b->freq.getFrequency());
        internal->left = (a->freq.getFrequency() <= b->freq.getFrequency()) ? a : b;
        internal->right = (internal->left == a) ? b : a;

        mergeQueue.push(internal);
        total = singleQueue.size() + mergeQueue.size();
    }
