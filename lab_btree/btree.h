...
private:
	...
	void printInorder(const BTreeNode* subroot) const;
};

//binary search helper for insertion_idx
// template <class T, class C>
// size_t binary_search(const std::vector<T>& elements, const C& val, size_t index) {
// 	if (elements.size() == 0) return index;
// 	if (elements.size() == 1) return (elements.at(0) < val) ? index + 1 : index;
...
// 	std::vector<T> temp;
// 	if (elements.at(mid) < val) {
// 		temp.assign(elements.begin() + mid + 1, elements.end());
// 		index += temp.size() / 2;
// 	}
// 	else {
// 		temp.assign(elements.begin(), elements.begin() + mid);
// 		index -= temp.size() / 2;
// 	}
// 	return binary_search(temp, val, index);
// }
...
size_t insertion_idx(const std::vector<T>& elements, const C& val) {
	...
	//binary search
	//return binary_search(elements, val, 0);
}
