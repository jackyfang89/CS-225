...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const {
    ...
    if (validIndex(subroot->elements, first_larger_idx)) {
...
template <class T>
bool validIndex(const std::vector<T> v, size_t index) {
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair) {
    ...
    // we only care if key DOESN'T exist
    bool keyExists = true;
    //check if key is valid, then check if key exists in subroot
    if (validIndex(subroot->elements, first_larger_idx)) {
...
template <class K, class V>
void BTree<K, V>::printInorder() const {
    printInorder(root);
}

template <class K, class V>
void BTree<K, V>::printInorder(const BTreeNode* subroot) const {
    // if (subroot->is_leaf) {
    //     std::cout << *subroot << " ";
    // }
    // else {
    //     for (size_t i = 0; i < (subroot->children.size() + 1)/2; ++i) {
    //         printInorder(subroot->children.at(i));
    //     }
    //     std::cout << *subroot << " ";
    //     for (size_t i = subroot->children.size()/2 + 1; i < subroot->children.size(); ++i) {
    //         printInorder(subroot->children.at(i));
    //     }
    // }
