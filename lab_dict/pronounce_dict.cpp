...
bool PronounceDict::homophones(const string& word1, const string& word2) const {
	...
	// map<string, vector<string>>::iterator it1 = dict.find(word1);
	// map<string, vector<string>>::iterator it2 = dict.find(word2);

	if (it1 == dict.end() || it2 == dict.end()) {
		return false;
	}

	vector<string> s1 = it1->second;
	vector<string> s2 = it2->second;
