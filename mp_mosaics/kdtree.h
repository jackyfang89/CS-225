...
public:
	...
	//public  function that calls quickselect for testing
	const Point<Dim> quickSelectP(vector<Point<Dim>>& newPoints, int k, int d) {
		return quickSelect(newPoints, 0, newPoints.size() - 1, k, d);
	}
...
private:
	...
	//helper funciton to calculate distance between two points
	double distance(const Point<Dim> a, const Point<Dim> b) const;

	//recursive helper function to perform quickSelect for constructor
	//returns the smallest kth point in a given vector of points
	Point<Dim> quickSelect(vector<Point<Dim>>& newPoints, int l, int r, int k, int d) const;

	//recursive helper function for quickselect in making partitions for quickSelect
	//partitions a vector such that all values left of the median value are less than median
	//thus partially sorting the vector
	int partition(vector<Point<Dim>>& vec, int l, int r, int d) const;

	//recursive helper function to help construct kd tree
	KDTreeNode* helpConstruct(vector<Point<Dim>> newPoints, int l, int r, int d);

	//recursive helper function for findNearestNeighbor
	//recursively traverses and finds the node in kd tree nearest to target
	Point<Dim> helpFind(const Point<Dim>& target, Point<Dim>& best, KDTreeNode* subroot, int d) const;

	void destroy(KDTreeNode* subroot);
	void copy(const KDTreeNode* other, KDTreeNode*& subroot);
};
